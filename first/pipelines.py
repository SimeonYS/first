...
class FirstPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `first`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM first WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO first (date, title, link, content)"
